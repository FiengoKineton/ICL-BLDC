...
def _run_model_on_experiment(
    ...
    """
    For a single experiment index in the Dataset, run the model and collect:
        t, omega_true, omega_hat, u_den
    All in *denormalized* units.

    Implementation: sliding window of length H = ds.seq_len,
    feeding back the model's own prediction into last_omega channel.
    This keeps t <= block_size at every forward call.
    """
    # Get full normalized experiment
    batch_u, batch_y = ds.get_full_experiment(idx)  # (T,5), (T,1)

    if batch_u.dim() != 2 or batch_u.size(1) != 5:
        raise ValueError(f"Expected batch_u of shape (T,5), got {tuple(batch_u.shape)}")
    if batch_y.dim() == 1:
        batch_y = batch_y.unsqueeze(-1)  # (T,1)
    elif batch_y.dim() != 2:
        raise ValueError(f"Expected batch_y of shape (T,1), got {tuple(batch_y.shape)}")

    batch_u = batch_u.to(device)  # (T,5)
    batch_y = batch_y.to(device)  # (T,1)
    ...
    y_pred = torch.zeros_like(batch_y, device=device)  # (T,1)

    # Window of last predicted omegas (normalized) for last_omega channel
    # Shape: (1, H, 1)
    last_omega = torch.zeros(1, H, 1, device=device)

    with torch.no_grad():
        for j in range(T):
            if j < H:
                window_u = batch_u[0:H, :]  # (H,5)
            else:
                window_u = batch_u[j - H + 1 : j + 1, :]  # (H,5)

            # Prepare input (1,H,5) and overwrite last_omega channel (index 4)
            window_u = window_u.unsqueeze(0).clone()  # (1,H,5)
            window_u[:, :, 4] = last_omega[:, :, 0]

            # Forward: output (1,H,1), take last time-step
            pred_last = model(window_u)[:, -1, :]  # (1,1)
            y_pred[j, 0] = pred_last[0, 0]

            # Update last_omega window
            last_omega = torch.roll(last_omega, shifts=-1, dims=1)
            last_omega[:, -1, 0] = pred_last[0, 0]

    # Denormalize
    batch_u_b = batch_u.unsqueeze(0)  # (1,T,5)
    batch_y_b = batch_y.unsqueeze(0)  # (1,T,1)
    y_pred_b = y_pred.unsqueeze(0)    # (1,T,1)

    batch_u_den, batch_y_den, y_pred_den = reverse_normalization(
        batch_u_b.cpu(), batch_y_b.cpu(), y_pred_b.cpu()
    )

    u_den = batch_u_den[0].numpy()            # (T,5)
    y_true = batch_y_den[0, :, 0].numpy()     # (T,)
